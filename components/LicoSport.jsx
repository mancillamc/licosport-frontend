...
async function getPresentaciones(productoId) {
  const r = await api("/presentaciones/producto/" + productoId);
  return r || [];
}
async function savePresentacion(data, id) {
  return api("/presentaciones" + (id ? "/" + id : ""), {method: id?"PUT":"POST", body:data});
}
async function deletePresentacion(id) {
  return api("/presentaciones/" + id, {method:"DELETE"});
}
...
function Login({onLogin}) {
  ...
  useEffect(()=>{ api("/vendedores").then(d=>{ if(d&&d.length) setVendedores(d); }); },[]);
...
function Toast({msg,type,onDone}) {
  ...
  return <div style={{position:"fixed",bottom:24,right:24,background:bg,color:"#fff",padding:"12px 20px",borderRadius:12,fontWeight:600,fontSize:14,zIndex:9999,boxShadow:"0 4px 16px #0003"}}>{msg}</div>;
}
  ...
  return (
    <aside style={{width:220,minHeight:"100vh",background:T.sidebarBg,display:"flex",flexDirection:"column",position:"fixed",left:0,top:0,bottom:0,zIndex:100,fontFamily:"system-ui,sans-serif"}}>
      <div style={{padding:"16px 16px 10px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><div style={{fontSize:14,fontWeight:800}}><span style={{color:T.sidebarAct}}>Lico</span><span style={{color:T.gold}}>Sport</span></div><div style={{color:T.sidebarText+"77",fontSize:9}}>Sistema de Gestión</div></div>
      </div>
...
function BottomNav({role,tab,onTab,alertas,credPend}) {
  ...
  return (
    <nav style={{position:"fixed",bottom:0,left:0,right:0,background:T.bgWhite,borderTop:`1px solid ${T.border}`,display:"flex",zIndex:800,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {nav.map(({id,icon,l})=>{
        ...
        return (
          <button key={id} onClick={()=>onTab(id)} style={{flex:1,border:"none",background:"transparent",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:isSell?0:"8px 0 6px",cursor:"pointer",position:"relative",fontFamily:"inherit"}}>
            {isSell?(<div style={{width:50,height:50,borderRadius:"50%",background:T.green,display:"flex",alignItems:"center",justifyContent:"center",fontSize:21,marginTop:-14,border:`3px solid ${T.bgPage}`}}>{icon}</div>):(
...
function Dialog({title,onClose,children}) {
  return (
    <div style={{position:"fixed",inset:0,background:"#0005",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:T.bgWhite,borderRadius:20,padding:24,width:"100%",maxWidth:520,maxHeight:"90vh",overflowY:"auto",border:`1px solid ${T.border}`,boxShadow:"0 8px 40px #0002"}}>
...
function Sheet({title,onClose,children}) {
  return (
    <div style={{position:"fixed",inset:0,background:"#0005",zIndex:1000,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:T.bgWhite,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:640,margin:"0 auto",maxHeight:"92vh",overflowY:"auto",border:`1px solid ${T.border}`,borderBottom:"none"}}>
...
function ModalPresentaciones({producto,onClose}) {
  ...
  useEffect(()=>{ getPresentaciones(producto.id).then(r=>{ if(r&&r.length) setLista(r); }); },[producto.id]);
  ...
  const guardar=async()=>{
    ...
    setLoading(true);
    const data={productoId:producto.id,nombre:form.nombre,equivaleUnidades:parseFloat(form.equivaleUnidades)||1,precioCompra:parseFloat(form.precioCompra)||0,precioVenta:parseFloat(form.precioVenta)||0,codigoBarras:form.codigoBarras||""};
  ...
  return (
    <Dialog title={`📦 Presentaciones — ${producto.nombre}`} onClose={onClose}>
            ...
            <div key={p.id} style={{background:T.bgSurface,borderRadius:10,padding:"10px 13px",marginBottom:8,border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{color:T.textMid,fontSize:11,marginTop:2}}>
                  Compra: {bs(p.precioCompra)} · Venta: <span style={{color:T.green}}>{bs(p.precioVenta)}</span> · Gan: <span style={{color:T.gold}}>+{bs(p.precioVenta-p.precioCompra)}</span>
                </div>
              </div>
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>editar(p)} style={{background:T.bgSurface,border:`1px solid ${T.border}`,borderRadius:8,padding:"4px 9px",cursor:"pointer",fontSize:11}}>✏️</button>
                <button onClick={()=>eliminar(p.id)} style={{background:T.redLight,border:`1px solid ${T.red}44`,color:T.red,borderRadius:8,padding:"4px 9px",cursor:"pointer",fontSize:11}}>🗑️</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{background:T.greenLight,borderRadius:12,padding:"14px 16px",border:`1px solid ${T.border}`}}>
        <div style={{color:T.greenText,fontWeight:700,fontSize:13,marginBottom:12}}>{editId?"✏️ Editar":"➕ Nueva presentación"}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <Fld label="Nombre" value={form.nombre} onChange={e=>setF("nombre",e.target.value)}/>
          <Fld label="Equivale (unidades)" type="number" value={form.equivaleUnidades} onChange={e=>setF("equivaleUnidades",e.target.value)}/>
          <Fld label="Precio compra Bs" type="number" value={form.precioCompra} onChange={e=>setF("precioCompra",e.target.value)}/>
          <Fld label="Precio venta Bs" type="number" value={form.precioVenta} onChange={e=>setF("precioVenta",e.target.value)}/>
        </div>
        <Fld label="Código de barras (opcional)" value={form.codigoBarras} onChange={e=>setF("codigoBarras",e.target.value)}/>
        <div style={{display:"flex",gap:9,marginTop:4}}>
          {editId&&<button onClick={resetForm} style={{padding:"10px 14px",background:T.bgWhite,border:`1px solid ${T.border}`,borderRadius:10,cursor:"pointer",fontSize:13}}>Cancelar</button>}
          <button onClick={guardar} disabled={loading} style={{flex:1,padding:"10px 14px",background:T.green,color:"#fff",border:"none",borderRadius:10,fontWeight:700,cursor:"pointer",fontSize:13}}>
    ...
    <Wrap title={prod?"✏️ Editar":"➕ Nuevo Producto"} onClose={onClose}>
      ...
      <div style={{marginBottom:13}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
          {CATS.map(({id,icon})=><button key={id} onClick={()=>set("cat",id)} style={{padding:"5px 11px",borderRadius:18,border:`1.5px solid ${f.cat===id?T.green:T.border}`,background:f.cat===id?T.greenLight:T.bgWhite,color:f.cat===id?T.greenText:T.textMid,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{icon} {id}</button>)}
        </div>
      </div>
      <Fld label="Unidad base" value={f.u} onChange={e=>set("u",e.target.value)}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <button onClick={()=>{ if(f.nombre.trim()) onSave(f); }} style={{width:"100%",padding:13,background:f.nombre.trim()?T.green:T.borderMid,color:"#fff",border:"none",borderRadius:12,fontWeight:700,fontSize:15,cursor:f.nombre.trim()?"pointer":"not-allowed",fontFamily:"inherit"}}>
...
function ModalVendedor({vend,onClose,onSave}) {
  ...
  return (
    <Dialog title={vend?"✏️ Editar Vendedor":"➕ Nuevo Vendedor"} onClose={onClose}>
      ...
      <Fld label="PIN (4 dígitos)" type="number" value={f.pin} onChange={e=>set("pin",e.target.value.slice(0,4))}/>
      ...
      <div style={{display:"flex",gap:9}}>
        ...
        <button onClick={handleSave} style={{flex:2,padding:12,background:f.nombre.trim()&&f.pin.length===4?T.green:T.borderMid,color:"#fff",border:"none",borderRadius:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
  ...
  return (
    <div>
      <div style={{marginBottom:18}}>
        ...
        <div style={{fontSize:22,fontWeight:800,color:T.text}}>{curUser?.icon} {curUser?.nombre}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:9,marginBottom:18}}>
        <button onClick={()=>onAction("venta")} style={{padding:"13px 10px",background:T.green,border:"none",borderRadius:13,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:7,fontFamily:"inherit"}}>🛒 Nueva venta</button>
        {isAdmin&&<button onClick={()=>onAction("nuevo")} style={{padding:"13px 10px",background:T.bgWhite,border:`1px solid ${T.border}`,borderRadius:13,color:T.text,fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:7,fontFamily:"inherit"}}>➕ Stock</button>}
        {isAdmin&&<button onClick={()=>onAction("inv")} style={{padding:"13px 10px",background:T.bgWhite,border:`1px solid ${T.border}`,borderRadius:13,color:T.text,fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:7,fontFamily:"inherit"}}>📦 Inventario</button>}
        {isAdmin&&<button onClick={()=>onAction("creditos")} style={{padding:"13px 10px",background:T.blueLight,border:`1px solid ${T.blue}44`,borderRadius:13,color:T.blue,fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:7,fontFamily:"inherit"}}>📋 Créditos</button>}
      </div>
      {ventas.length>0&&(
        <div>
            ...
            <Card key={v.id} style={{marginBottom:8,padding:"11px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{color:T.textMid,fontSize:10,marginTop:2}}>{v.hora} · {v.vendedor}</div>
                </div>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:7,marginBottom:13}}>
        <span style={{background:T.bgSurface,color:T.text,fontSize:11,fontWeight:600,padding:"3px 11px",borderRadius:18,border:`1px solid ${T.border}`}}>{productos.length} total</span>
        <span style={{background:T.goldLight,color:T.gold,fontSize:11,fontWeight:600,padding:"3px 11px",borderRadius:18,border:`1px solid ${T.border}`}}>{productos.filter(p=>p.stock<=p.min&&p.stock>0).length} bajo</span>
        <span style={{background:T.redLight,color:T.red,fontSize:11,fontWeight:600,padding:"3px 11px",borderRadius:18,border:`1px solid ${T.border}`}}>{productos.filter(p=>p.stock===0).length} agotado</span>
      </div>
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:T.bgSurface,borderBottom:`2px solid ${T.border}`}}>
              {["Producto","Cat.","Stock","Compra","Venta","Ganancia","%","Estado",""].map(h=><th key={h} style={{padding:"10px 12px",color:T.textMid,textAlign:"left",fontWeight:600,fontSize:11,whiteSpace:"nowrap"}}>{h}</th>)}
            </tr></thead>
            <tbody>
              {lista.map((p,i)=>{
                  ...
                  <tr key={p.id} style={{borderBottom:`1px solid ${T.border}`,background:i%2===0?T.bgWhite:T.bgSurface}}>
                    ...
                    <td style={{padding:"10px 12px"}}>
                      <div style={{display:"flex",gap:5}}>
                        <button onClick={()=>onPresentaciones(p)} style={{background:T.blueLight,border:`1px solid ${T.blue}44`,color:T.blue,borderRadius:7,padding:"4px 9px",cursor:"pointer",fontSize:10,whiteSpace:"nowrap"}}>📦 Present.</button>
                        <button onClick={()=>onEditar(p)} style={{background:T.bgSurface,border:`1px solid ${T.border}`,color:T.text,borderRadius:7,padding:"4px 9px",cursor:"pointer",fontSize:11}}>✏️</button>
                        <button onClick={()=>onEliminar(p.id)} style={{background:T.redLight,border:`1px solid ${T.red}44`,color:T.red,borderRadius:7,padding:"4px 9px",cursor:"pointer",fontSize:11}}>🗑️</button>
                      </div>
                    </td>
                  </tr>
                );
...
function Venta({role,curUser,productos,onConfirmar}) {
  ...
  const selectProd=async(p)=>{
    setSelProd(p);
    setBusq(p.nombre);
    setPresentaciones([{id:"default",nombre:"Unidad",equivaleUnidades:1,precioCompra:p.pc||0,precioVenta:p.pv||0}]);
  ...
  const addMore=(item)=>setCarrito(c=>{
    const unidsTotal=c.filter(i=>i.prodId===item.prodId).reduce((s,i)=>s+i.qty*(i.equivUnids||1),0);
    if(unidsTotal+(item.equivUnids||1)>item.stock) return c;
        ...
        <div>
          {!selProd&&(
            <div>
              ...
              <div style={{maxHeight:380,overflowY:"auto"}}>
                {busq&&disp.map(p=>(
                  <div key={p.id} onClick={()=>selectProd(p)} style={{marginBottom:7,padding:"11px 13px",cursor:"pointer",background:T.bgWhite,border:`1.5px solid ${T.border}`,borderRadius:16}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          {selProd&&(
            <div>
              ...
              <div style={{background:T.greenLight,borderRadius:16,border:`1px solid ${T.border}`,padding:"15px 17px",marginBottom:12}}>
              ...
              {presentaciones.map(pres=>(
                <div key={pres.id} onClick={()=>addToCart(selProd,pres)} style={{marginBottom:9,cursor:"pointer",background:T.bgWhite,border:`1.5px solid ${T.border}`,borderRadius:16,padding:"15px 17px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                    ...
                    <div style={{textAlign:"right"}}>
                      <div style={{color:T.green,fontWeight:900,fontFamily:"monospace",fontSize:16}}>{bs(pres.precioVenta||0)}</div>
                ...
                <div key={i.key} style={{marginBottom:7,padding:"9px 11px",background:T.bgWhite,border:`1px solid ${T.border}`,borderRadius:16}}>
                  <div style={{display:"flex",alignItems:"center",gap:7}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      ...
                      <button onClick={()=>addMore(i)} style={{width:26,height:26,borderRadius:"50%",background:T.bgSurface,border:`1px solid ${T.border}`,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:T.green}}>+</button>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{textAlign:"right"}}>
                  ...
                  <div style={{display:"flex",gap:6,marginTop:6}}>
                    <button onClick={()=>onPagar(c.id)} style={{padding:"5px 12px",background:T.green,border:"none",color:"#fff",borderRadius:8,fontWeight:700,cursor:"pointer",fontSize:12}}>✅ Cobrar</button>
                    <button onClick={()=>onEliminar(c.id)} style={{padding:"5px 10px",background:T.redLight,border:`1px solid ${T.red}44`,color:T.red,borderRadius:8,cursor:"pointer",fontSize:12}}>🗑️</button>
                  </div>
                </div>
              </div>
            </Card>
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            {topR.map((p,i)=>(
              <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",borderBottom:i<4?`1px solid ${T.border}`:"none",alignItems:"center"}}>
                <div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:15}}>{["🥇","🥈","🥉","4️⃣","5️⃣"][i]}</span><div><div style={{color:T.text,fontSize:12,fontWeight:600}}>{p.nombre}</div></div></div>
        ...
        <div>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            {porCat.map((c,i)=>(
              <div key={c.id} style={{padding:"9px 14px",borderBottom:i<porCat.length-1?`1px solid ${T.border}`:"none"}}>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            {[...ventas].reverse().map((v,i)=>(
              <div key={v.id} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",borderBottom:i<ventas.length-1?`1px solid ${T.border}`:"none",alignItems:"center"}}>
                ...
                <div style={{textAlign:"right"}}><div style={{color:T.green,fontWeight:700,fontFamily:"monospace",fontSize:12}}>{bs(v.total)}</div><div style={{color:T.gold,fontSize:10,fontFamily:"monospace"}}>+{bs(v.gan)}</div></div>
              </div>
            ))}
          </Card>
        }
          ...
          {[...mias].reverse().map((v,i)=>(
            <div key={v.id} style={{padding:"10px 14px",borderBottom:i<mias.length-1?`1px solid ${T.border}`:"none",display:"flex",justifyContent:"space-between"}}>
              <div>
                ...
                <div style={{color:T.textMid,fontSize:10,marginTop:2}}>{v.hora}</div>
              </div>
              <div style={{color:T.green,fontWeight:700,fontFamily:"monospace",fontSize:13}}>{bs(v.total)}</div>
            </div>
          ))}
        </Card>
      }
...
function Usuarios({showToast}) {
  ...
  const guardar=async(f)=>{
    if(f.id){
      const r=await api(`/vendedores/${f.id}`,{method:"PUT",body:f});
      if(r){setVendedores(vs=>vs.map(v=>v.id===f.id?r:v));showToast("Vendedor actualizado");}
      else showToast("Error al actualizar","error");
    } else {
      const r=await api("/vendedores",{method:"POST",body:f});
      if(r){setVendedores(vs=>[...vs,r]);showToast("Vendedor creado");}
      else showToast("Error — PIN puede estar en uso","error");
    }
    setModal(null);
  };
      ...
      {vendedores.map(v=>(
        <Card key={v.id} style={{marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{display:"flex",gap:7}}>
            <button onClick={()=>setModal(v)} style={{background:T.bgSurface,border:`1px solid ${T.border}`,color:T.text,borderRadius:9,padding:"6px 12px",cursor:"pointer",fontSize:12}}>✏️ Editar</button>
            <button onClick={()=>eliminar(v.id)} style={{background:T.redLight,border:`1px solid ${T.red}44`,color:T.red,borderRadius:9,padding:"6px 10px",cursor:"pointer",fontSize:12}}>🗑️</button>
          </div>
        </Card>
      ))}
      {vendedores.length===0&&<Card><div style={{color:T.textLight,textAlign:"center",padding:28}}>No hay vendedores.</div></Card>}
  ...
  return (
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <button onClick={cambiar} style={{padding:"10px 20px",background:T.green,border:"none",color:"#fff",borderRadius:10,fontWeight:700,cursor:"pointer"}}>Guardar PIN</button>
      </Card>
    </div>
  );
}
...
export default function LicoSport() {
  ...
  const handleLogin=useCallback(async(r,user,token)=>{
    ...
    const tok=token||(typeof window!=="undefined"?localStorage.getItem("ls_tok"):null);
    if(tok){
      try{
        const res=await fetch(API_URL+"/productos",{headers:{Authorization:"Bearer "+tok,"Content-Type":"application/json"}});
        if(res.ok){const data=await res.json();if(data&&data.length>0){setProductos(data);setApiOk(true);}}
      }catch(e){}
    }
  },[]);
  ...
  const confirmarVenta=(carrito,metodo,cliente)=>{
    ...
    setProductos(ps=>ps.map(p=>{
      const unids=carrito.filter(i=>i.prodId===p.id).reduce((s,i)=>s+i.qty*(i.equivUnids||1),0);
      return unids>0?{...p,stock:Math.max(0,p.stock-unids)}:p;
    }));
    const v={id:Date.now(),hora:new Date().toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit"}),vendedor:curUser?.nombre||"?",metodoPago:metodo,items:carrito.map(i=>({nombre:i.nombre,presentacion:i.presentacion,qty:i.qty,pvPres:i.pvPres,pcPres:i.pcPres})),total,gan:gan_};
    setVentas(vs=>[...vs,v]);
    if(metodo==="credito"){
      setCreditos(cs=>[...cs,{id:Date.now(),cliente,monto:total,itemsStr:carrito.map(i=>`${i.nombre} ×${i.qty}`).join(", ").slice(0,50),hora:v.hora,vendedor:curUser?.nombre||"?",pagado:false}]);
      showToast(`📋 Crédito para ${cliente} · ${bs(total)}`,"warning");
    } else {
      showToast(`✅ Venta · ${bs(total)}`);
    }
    setTab("home");
  };
