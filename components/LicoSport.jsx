...
function Login({ onLogin }) {
  ...
  useEffect(() => {
    api("/vendedores").then(d => { if (d?.length) setVendedores(d); });
  }, []);

  const goPin = (admin, vend = null) => {
    setIsAdmin(admin); setSelVend(vend);
    setStep("pin"); setDigits([]); setErr("");
  };
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        <div style={{ textAlign:"center", marginBottom:28 }}>
          <img
            src="/logo.png"
            alt="LicoSport"
            style={{ width:160, height:160, objectFit:"contain", display:"block", margin:"0 auto 10px", borderRadius:20 }}
          />
        ...
        <div style={{ background:T.bgWhite, borderRadius:20, padding:24, boxShadow:"0 2px 20px #00000010", border:`1px solid ${T.border}` }}>
          ...
          {step === "role" && (
            <>
              ...
              {vendedores.length === 0 && (
                <div style={{ color:T.textLight, fontSize:12, textAlign:"center", padding:"10px 0" }}>
                  Cargando vendedores...
                </div>
              )}
            </>
          ...
          {step === "pin" && (
            <>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20, padding:"12px 14px", background:roleColor+"14", borderRadius:12 }}>
                ...
                <div>
                  <div style={{ color:roleColor, fontWeight:700, fontSize:14 }}>{isAdmin ? "Administrador" : selVend?.nombre}</div>
                  <div style={{ color:T.textMid, fontSize:12 }}>Ingresa tu PIN de 4 dígitos</div>
                </div>
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:9 }}>
                {KEYS.map((k, i) => {
                  ...
                  const isDel = k === "⌫";
                  return (
                    <button key={i} onClick={() => isDel ? del() : press(k)} disabled={loading}
                      style={{ height:56, borderRadius:14, background: isDel ? T.bgSurface : T.bgWhite, border:`1.5px solid ${T.border}`, color: isDel ? T.red : T.text, fontSize:21, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
              ...
              <div style={{ color:T.textLight, fontSize:11, textAlign:"center", marginTop:14 }}>
                Admin PIN: 1234
              </div>
            </>
          )}
...
function Toast({ msg, type, onDone }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:24, right:24, background:bg, color:"#fff", padding:"12px 20px", borderRadius:12, fontWeight:600, fontSize:14, zIndex:9999, boxShadow:"0 4px 16px #0003", animation:"sIn .25s ease" }}>
      {msg}
...
const NAV_ADMIN = [
  ...
  {id:"inv",      icon:"📦", l:"Inventario"},
  {id:"venta",    icon:"🛒", l:"Nueva Venta"},
  {id:"creditos", icon:"📋", l:"Créditos"},
  {id:"report",   icon:"📊", l:"Reportes"},
  {id:"usuarios", icon:"👥", l:"Vendedores"},
  {id:"config",   icon:"⚙️", l:"Config"},
];
const NAV_VEND = [
  {id:"home",     icon:"🏠", l:"Inicio"},
  {id:"venta",    icon:"🛒", l:"Vender"},
  {id:"historial",icon:"🧾", l:"Mis Ventas"},
];
  ...
  return (
    <aside style={{ width:220, minHeight:"100vh", background:T.sidebarBg, display:"flex", flexDirection:"column", position:"fixed", left:0, top:0, bottom:0, zIndex:100, fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ padding:"16px 16px 10px", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          <div style={{ fontSize:14, fontWeight:800, letterSpacing:-.3 }}>
            <span style={{ color:T.sidebarAct }}>Lico</span><span style={{ color:T.gold }}>Sport</span>
          </div>
          <div style={{ color:T.sidebarText+"77", fontSize:9 }}>Sistema de Gestión</div>
        </div>
      </div>
      ...
      <div style={{ margin:"0 10px 14px", background:"#FFFFFF18", borderRadius:12, padding:"10px 12px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:9 }}>
          ...
          <div>
            <div style={{ color:"#fff", fontWeight:700, fontSize:12 }}>{curUser?.nombre}</div>
...
function Dialog({ title, onClose, children }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"#0005", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ background:T.bgWhite, borderRadius:20, padding:24, width:"100%", maxWidth:500, maxHeight:"90vh", overflowY:"auto", border:`1px solid ${T.border}`, boxShadow:"0 8px 40px #0002", animation:"shUp .2s ease" }}>
...
function ModalProd({ prod, isMob, onClose, onSave }) {
  ...
  return (
    <Wrap title={prod?"✏️ Editar":"➕ Nuevo Producto"} onClose={onClose}>
      ...
      <Fld label="Unidad (und./bot./caja)" value={f.u} onChange={e=>set("u",e.target.value)} />
  ...
  return (
    <Dialog title={vend?"✏️ Editar Vendedor":"➕ Nuevo Vendedor"} onClose={onClose}>
      <div style={{ display:"flex", justifyContent:"center", marginBottom:16 }}>
        <div style={{ width:56, height:56, borderRadius:16, background:f.color+"22", color:f.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:800 }}>
          {f.nombre ? initials(f.nombre) : "?"}
        </div>
      </div>
      <Fld label="Nombre del vendedor" value={f.nombre} onChange={e=>set("nombre",e.target.value)} />
      <div style={{ marginBottom:13 }}>
        <label style={{ color:T.textMid, fontSize:11, display:"block", marginBottom:7, textTransform:"uppercase", letterSpacing:.7 }}>Color de identificación</label>
        <div style={{ display:"flex", gap:9, flexWrap:"wrap" }}>
          {COLORS_V.map(c => (
            <button key={c} onClick={() => set("color",c)} style={{ width:30, height:30, borderRadius:"50%", background:c, border: f.color===c?"3px solid #000":"2px solid transparent", cursor:"pointer" }} />
          ))}
        </div>
      </div>
          ...
          {[...ventas].reverse().slice(0,4).map(v => (
            <Card key={v.id} style={{ marginBottom:8, padding:"11px 14px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <div style={{ color:T.text, fontSize:12 }}>{v.items.map(i=>`${i.nombre} ×${i.qty}`).join(", ").slice(0,48)}</div>
                  <div style={{ color:T.textMid, fontSize:10, marginTop:2 }}>
                    {v.hora} · {v.vendedor}
                    {v.metodoPago && <span style={{ marginLeft:7, background:v.metodoPago==="credito"?T.blueLight:T.greenLight, color:v.metodoPago==="credito"?T.blue:T.green, fontSize:9, fontWeight:700, padding:"1px 7px", borderRadius:8 }}>{PAGOS.find(m=>m.id===v.metodoPago)?.label}</span>}
                  </div>
                </div>
...
function Venta({ role, curUser, productos, onConfirmar }) {
  ...
  const ganT    = carrito.reduce((s,i)=>s+(i.pv-i.pc)*i.qty,0);
  const esCred  = metodo==="credito";

  const add  = (p) => setCarrito(c=>{const ex=c.find(i=>i.id===p.id);if(ex)return ex.qty>=p.stock?c:c.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i);return[...c,{...p,qty:1}];});
  const less = (id) => setCarrito(c=>{const ex=c.find(i=>i.id===id);if(!ex||ex.qty<=1)return c.filter(i=>i.id!==id);return c.map(i=>i.id===id?{...i,qty:i.qty-1}:i);});
  const rem  = (id) => setCarrito(c=>c.filter(i=>i.id!==id));

  const confirmar = () => {
    if (esCred && !cliente.trim()) { alert("Ingresa el nombre del cliente para crédito"); return; }
        ...
        <div>
          ...
          <div style={{ maxHeight:380, overflowY:"auto" }}>
            {busq && disp.map(p => {
              ...
              return (
                <Card key={p.id} style={{ marginBottom:7, padding:"11px 13px", cursor:"pointer", border:`1.5px solid ${enC?T.green+"66":T.border}` }} onClick={()=>add(p)}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <div style={{ textAlign:"right" }}><div style={{ color:T.green, fontWeight:800, fontFamily:"monospace" }}>{bs(p.pv)}</div>{enC&&<div style={{ color:T.gold, fontSize:10 }}>×{enC.qty}</div>}</div>
                  </div>
                </Card>
              );
            })}
                ...
                <Card key={i.id} style={{ marginBottom:7, padding:"9px 11px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      <button onClick={()=>less(i.id)} style={{ width:26,height:26,borderRadius:"50%",background:T.bgSurface,border:`1px solid ${T.border}`,color:T.text,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit" }}>−</button>
                      <span style={{ color:T.text,fontWeight:700,minWidth:16,textAlign:"center",fontSize:12 }}>{i.qty}</span>
                      <button onClick={()=>add(i)} style={{ width:26,height:26,borderRadius:"50%",background:T.bgSurface,border:`1px solid ${T.border}`,color:T.green,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit" }}>+</button>
                      <button onClick={()=>rem(i.id)} style={{ width:26,height:26,borderRadius:"50%",background:T.redLight,border:"none",color:T.red,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit" }}>×</button>
                    </div>
                  </div>
                </Card>
              ))}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))", gap:9 }}>
            {porM.map(m=>(
              <Card key={m.id} style={{ background:m.id==="credito"?T.blueLight:T.greenLight }}>
                ...
                <div style={{ color:m.id==="credito"?T.blue:T.green, fontWeight:800, fontSize:14, fontFamily:"monospace" }}>{bs(m.total)}</div>
...
// GESTIÓN VENDEDORES (admin)
function Usuarios({ showToast }) {
  const [vendedores, setVendedores] = useState([]);
  const [modal,      setModal]      = useState(null); // null | {} | vendedor

  useEffect(() => {
    api("/vendedores").then(d => { if (d) setVendedores(d); });
  }, []);
  ...
  const eliminar = async (id) => {
    ...
    showToast("Vendedor eliminado","error");
  };
    ...
    <div>
      ...
      {vendedores.map(v => (
        <Card key={v.id} style={{ marginBottom:10, display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:44, height:44, borderRadius:12, background:v.color+"22", color:v.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:800, flexShrink:0 }}>
            {initials(v.nombre)}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ color:T.text, fontWeight:700, fontSize:14 }}>{v.nombre}</div>
            <div style={{ color:T.textMid, fontSize:11, marginTop:2 }}>{v.turno || "Sin turno"} · PIN: ••••</div>
          </div>
      ...
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, fontSize:12 }}>
          <div>
            ...
            {["Inventario completo","Precios de compra y ganancias","Gestionar vendedores","Reportes completos","Gestionar créditos","Cambiar PINs"].map(p=>(
          ...
          <div>
            ...
            {["Registrar ventas","Elegir método de pago","Ver historial propio","Precios de venta"].map(p=>(
              <div key={p} style={{ display:"flex", gap:5, marginBottom:3 }}><span style={{ color:T.green }}>✓</span><span style={{ color:T.text }}>{p}</span></div>
            ))}
          </div>
        </div>
...
export default function LicoSport() {
  ...
  const confirmarVenta = (carrito, metodo, cliente) => {
    ...
    setProductos(ps=>ps.map(p=>{const it=carrito.find(i=>i.id===p.id);return it?{...p,stock:p.stock-it.qty}:p;}));
    const v={id:Date.now(),hora:new Date().toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit"}),vendedor:curUser?.nombre||"?",metodoPago:metodo,items:carrito,total,gan:gan_};
    setVentas(vs=>[...vs,v]);
    if (metodo==="credito") {
  ...
  const content = () => {
    switch(tab) {
      ...
      case "inv":      return role==="admin" ? <Inventario productos={productos} onEditar={setModalProd} onEliminar={eliminarProd} onNuevo={()=>setModalProd({})} /> : null;
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bgPage, fontFamily:"system-ui,-apple-system,sans-serif", color:T.text }}>
      {!apiOk && (
        <div style={{ background:T.goldLight, borderBottom:`1px solid ${T.gold}44`, padding:"5px 16px", textAlign:"center", fontSize:11, color:T.gold }}>
          ⚡ Modo offline — conectando al servidor...
        </div>
      )}
